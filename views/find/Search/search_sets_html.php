...
<div id="searchSetTools">
	...
	<div class="row">
		<div class="col">
			<div class="searchSetsToggle">
				<a href="#" onclick="return caToggleAddToSet();" class="searchSetsToggle"><?php print _t("Toggle checked"); ?></a>
			</div>
		</div>
	</div>
<?php
...
?>	
	<div class="row">
		<div class="col">
	<?php
		print "<h5 class='header'>"._t("Add checked to set").":</h5>";
	?>
		<form id="caAddToSet">
	...
	<?php
			...
			foreach($va_sets as $vn_set_id => $va_set_info) {
				$va_options[$va_set_info['name']."(".$vn_set_id.")"] = $vn_set_id;
			}
		
			print caHTMLSelect('set_id', $va_options, array('id' => 'caAddToSetID', 'class' => 'searchSetsSelect'), array('value' => null));
			print caBusyIndicatorIcon($this->request, array('id' => 'caAddToSetIDIndicator'))."\n";
	?>
				</div>
				<a href='#' onclick="return caAddItemsToSet();" class="btn btn-outline-primary btn-block"><?php print _t('Add'); ?> &rsaquo;</a>
			</form>
		</div>
	</div>
	<hr>
<?php
	...
	if($vb_show_create_set_from_checked) {
		?>
			...
			<div class="col">
		<?php
			print "<h5 class='header'>"._t("Create set").":</h5>";
		?>
			<form id="caCreateSetFromResults">
			<div class="form-group">
		<?php
				print caHTMLTextInput('set_name', array('id' => 'caCreateSetFromResultsInput', 'class' => 'searchSetsTextInput', 'value' => $o_result_context->getSearchExpression()), array());
		?>
				</div>
				<div class="form-group">
		<?php
		...
		<?php
			...
			?>
				...
				<a href='#' onclick="caCreateSetFromResults(); return false;" class="btn btn-outline-primary btn-block"><?php print _t('Create'); ?> &rsaquo;</a>
			</form>
		</div>
		</div>
<?php
