<?php
/** ---------------------------------------------------------------------
 * mana/helpers/displayHelpers.php : miscellaneous functions
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
require_once(__CA_LIB_DIR__.'/core/Parsers/DisplayTemplateParser.php');
	...
	function manaGetUserLocaleRules($ps_item_locale=null, $pa_preferred_locales=null) {
	 ...
	 */
	function manaExtractValuesByLocale($pa_locale_rules, $pa_values, $pa_options=null) {
	...
	# ------------------------------------------------------------------------------------------------
	function manaExtractValuesByUserLocale($pa_values, $ps_item_locale=null, $pa_preferred_locales=null, $pa_options=null) {
	 ...
	 */
	function manaExtractValuesByUserLocaleFromHierarchyAncestorList($pa_list, $ps_primary_key_name, $ps_label_display_field, $ps_use_if_no_label_field, $ps_default_text='???') {
	...
	# ------------------------------------------------------------------------------------------------
	function manaExtractValuesByUserLocaleFromHierarchyChildList($pa_list, $ps_primary_key_name, $ps_label_display_field, $ps_use_if_no_label_field, $ps_default_text='???') {
		if (!is_array($pa_list)) { return array(); }
		$va_values = array();
		foreach($pa_list as $vn_i => $va_item) {
	...
	# ------------------------------------------------------------------------------------------------
	function manaFormatFieldErrorsAsHTML($pa_errors, $ps_css_class) {
	...
	# ------------------------------------------------------------------------------------------------
	function manaFormControlBox($ps_left_content, $ps_middle_content, $ps_right_content, $ps_second_row_content='') {
	 ...
	 *
	 */
	function manaDeleteWarningBox($po_request, $t_instance, $ps_item_name, $ps_module_path, $ps_controller, $ps_cancel_action, $pa_parameters) {
	 ...
	 *
	 */
	function manaDeleteRemapper($po_request, $t_instance) {
	 ...
	 */
	function manaBusyIndicatorIcon($po_request, $pa_attributes=null) {
	 ...
	 */
	function manaFormatMediaMetadata($pa_metadata) {
		$vs_buf = "<table>\n";
			...
			foreach($pa_metadata as $vs_metadata_type => $va_metadata_data) {
				if (isset($va_metadata_data) && is_array($va_metadata_data)) {
					...
					foreach($va_metadata_data as $vs_key => $vm_value) {
						$vs_buf .=  "<tr valign='top'><td><!-- empty --></td><td>{$vs_key}</td><td>"._manaFormatMediaMetadataArray($vm_value, 0, $vs_key)."</td></tr>\n";
						$vn_metadata_rows++;
					}
				}
			}
	 ...
	 * @return string - formated metadata for display to user
	 */
	function _manaFormatMediaMetadataArray($pa_array, $pn_level=0, $ps_key=null) {
		...
		foreach($pa_array as $vs_key => $vs_val) {
			...
			$vs_buf .= "<tr><td width='130'>{$vs_key}</td><td>"._manaFormatMediaMetadataArray($vs_val, $pn_level + 1, $vs_key)."</td></tr>";
		}
		$vs_buf .= "</table></div>\n";
		return $vs_buf;
	}
	 ...
	 */
	function manaEditorFindResultNavigation($po_request, $po_instance, $po_result_context, $pa_options=null) {
	 ...
	 * @return string 
	 */
	function manaSetupEditorScreenOverlays($po_request, $pt_subject, $pa_bundle_list, $pa_options=null) {
		$vs_buf = '';
	 ...
	 * @return string 
	 */
	function manaEditorFieldList($po_request, $pt_subject, $pa_bundle_list, $pa_options=null) {
...
<div id=\"editorFieldListHTML\">";
	 ...
	 * @return string 
	 */
	function manaEditorHierarchyOverview($po_request, $ps_table, $pn_id, $pa_options=null) {
		$o_dm = Datamodel::load();
...
\n";
	 ...
	 */
	function manaEditorInspector($po_view, $pa_options=null) {
		...
		if (!$vs_color) { $vs_color = "#fffff"; }
		
		$vs_buf .= "<h4><div id='caColorbox' >\n";
		...
		if ($t_item->hasField('is_deaccessioned') && $t_item->get('is_deaccessioned') && ($t_item->get('deaccession_date', array('getDirectDate' => true)) <= caDateToHistoricTimestamp(_t('now')))) {
			...
			if ($po_view->request->user->canDoAction('can_see_current_location_in_inspector_ca_objects')) {
				if (($t_ui && method_exists($t_item, "getObjectHistory")) && (is_array($va_placements = $t_ui->getPlacementsForBundle('ca_objects_history')) && (sizeof($va_placements) > 0))) {
					...
					if (is_array($va_history = $t_item->getObjectHistory($va_bundle_settings, array('limit' => 1, 'currentOnly' => true))) && (sizeof($va_history) > 0)) {
						...
						if ($va_current_location['display']) { $vs_buf .= "<div class='lead'><strong>".$vs_inspector_current_location_label.':</strong><br/>'.$va_current_location['display']."</div>"; }
					}
					...
					if ($vs_current_location = $t_item->getLastLocationForDisplay("<l><ifdef code='ca_storage_locations.parent.preferred_labels'>^ca_storage_locations.parent.preferred_labels > </ifdef>^ca_storage_locations.preferred_labels.name</l>")) {
						$vs_buf .= "<div class='lead pb-2'><strong>"._t('Location').":</strong>".$vs_current_location."</div>\n";
						...
						if ($vs_full_location_hierarchy !== $vs_current_location) { TooltipManager::add(".lead", $vs_full_location_hierarchy); }
					}
				}
			}
		}
			...
			if (($vs_table_name === 'ca_object_lots') && $t_item->getPrimaryKey()) {
				...
				if(!(bool)$po_view->request->config->get('disable_add_object_to_lot_inspector_controls')) {
					...
					$vs_buf .= "<script type='text/javascript'>
	function manaAddObjectToLotForm() { 
	 ...
	 * @return string HTML implementing the inspector
	 */
	function manaEditorACLEditor($po_view, $pt_instance, $pa_options=null) {
	 ...
	 * @return string HTML implementing the inspector
	 */
	function manaBatchEditorInspector($po_view, $pa_options=null) {
		require_once(__CA_MODELS_DIR__.'/ca_sets.php');
	 ...
	 * @return string HTML implementing the inspector
	 */
	function manaBatchMediaImportInspector($po_view, $pa_options=null) {
		$vs_color = "444444"; 
		$vs_buf .= "<h4><div id='caColorbox' style='border: 6px solid #{$vs_color}; padding-bottom:15px;'>\n";
	 ...
	 * @return string HTML implementing the inspector
	 */
	function manaBatchMetadataExportInspector($po_view) {
		$vs_color = "444444";
		$vs_buf = "<h4><div id='caColorbox' style='border: 6px solid #{$vs_color}; padding-bottom:15px;'>\n";
	...
	/**
	  *
	  */
	function manaTableIsActive($pm_table) {
		$o_dm = Datamodel::load();
	...
	/**
	  *
	  */
	function manaFilterTableList($pa_tables, $pa_options=null) {
	 ...
	 *
	 */
	function manaGetTableDisplayName($pm_table_name_or_num, $pb_use_plural=true) {
		require_once(__CA_MODELS_DIR__.'/ca_occurrences.php');
		$o_dm = Datamodel::load();
	 ...
	 * @return 
	 */
	function manaGetMediaDisplayInfo($ps_context, $ps_mimetype) {
		$o_config = Configuration::load();
	 ...
	 * @return 
	 */
	function manaGetDefaultMediaViewer($ps_mimetype) {
		$o_config = Configuration::load();
		$o_media_display_config = Configuration::load(__CA_APP_DIR__.'/conf/media_display.conf');
	 ...
	 */
	function manaGetTemplateTags($ps_template, $pa_options=null) {
	 ...
	 */
	function manaProcessTemplate($ps_template, $pa_values, $pa_options=null) {
	 ...
	 * @return string
	 */
	function manaProcessTemplateTagDirectives($ps_value, $pa_directives) {
	    global $g_ui_locale;
	 ...
	 */
	function manaProcessTemplateForIDs($ps_template, $pm_tablename_or_num, $pa_row_ids, $pa_options=null) {
	 ...
	 * @return string 
	 */
	function manaGetRelationDisplayString($po_request, $ps_table, $pa_attributes=null, $pa_options=null) {
		$o_config = Configuration::load();
		$o_dm = Datamodel::load();
	 ...
	 */
	function manaGetLocalizedDate($pn_timestamp=null, $pa_options=null) {
	 ...
	 * @return string Localized date/time expression
	 */
	function manaGetLocalizedHistoricDate($pn_timestamp=null, $pa_options=null) {
	 ...
	 * @return string Localized date/time expression
	 */
	function manaGetLocalizedHistoricDateRange($pn_start_timestamp, $pn_end_timestamp, $pa_options=null) {
		$o_tep = new TimeExpressionParser();
	 ...
	 */
	function manaGetLocalizedDateRange($pn_start_timestamp, $pn_end_timestamp, $pa_options=null) {
		$o_tep = new TimeExpressionParser();
	 ...
	 */
	function manaNormalizeDateRange($ps_expression, $ps_normalization, $pa_options=null) {
		$o_tep = new TimeExpressionParser();
	 ...
	 * @return array
	 */
	function manaGetDateRangeForTimelineJS($pa_historic_timestamps) {
		$o_tep = new TimeExpressionParser();
     ...
     * @return string Localized date range expression
     */
    function manaGetDateRangeForCalendar($pa_historic_timestamps, $pa_options=null) {
        $o_tep = new TimeExpressionParser();
	 ...
	 */
	function manaGetRepresentationDimensionsForDisplay($po_rep, $ps_version, $pa_options=null) {
	 ...
	 * @return string 
	 */
	function manaFormatFileSize($pn_bytes) {
	 ...
	 * @return mixed 
	 */
	function manaProcessRelationshipLookupLabel($qr_rel_items, $pt_rel, $pa_options=null) {
	 ...
	 *
	 */
	function manaGetMediaMimetypeToDisplayClassMap($ps_context) {
		$o_config = Configuration::load();
	 ...
	 *
	 */
	function manaObjectsDisplayDownloadLink($po_request, $pn_object_id = null) {
		$o_config = Configuration::load();
	 ...
	 */
	function manaCreateLinksFromText($pa_text, $ps_table_name, $pa_row_ids, $ps_class=null, $ps_target=null, $pa_options=null) {
	 ...
	 * @return string
	 */
	function manaGetBundleDisplayTemplate($pt_subject, $ps_related_table, $pa_bundle_settings, $pa_options=null) {
		$vs_template = null;
	 ...
	 */
	function manaEditorBundleShowHideControl($po_request, $ps_id_prefix, $pa_settings=null, $pb_has_value=false, $ps_preview_init="&nbsp;") {
	 ...
	 * @return string HTML implementing the control
	 */
	function manaEditorBundleMetadataDictionary($po_request, $ps_id_prefix, $pa_settings) {
		global $g_ui_locale;
	 ...
	 * @return string HTML implementing the control
	 */
	function manaEditorBundleSortControls($po_request, $ps_id_prefix, $ps_table, $pa_options=null) {
	 ...
	 * 
	 */
	function manaProcessBottomLineTemplateForDisplay($po_request, $pt_display, $pr_res, $pa_options=null) {
	 ...
	 * 
	 */
	function manaProcessBottomLineTemplateForPlacement($po_request, $pa_placement, $pr_res, $pa_options=null) {
	 ...
	 *
 	 ...
 	 */
 	function manaRepresentationViewer($po_request, $po_data, $pt_subject, $pa_options=null) {
	 ...
	 */
	function manaRepToolbar($po_request, $pt_representation, $pt_subject, $pa_options=null){
	 ...
	 */
	function manaGetMediaIdentifier($po_request) {		
	 ...
	 *
	 */
	function manaGetMediaAnnotationList($po_data, $pa_options=null) {
		$ps_annotation_display_template 	= caGetOption('displayAnnotationTemplate', $pa_options, caGetOption('displayAnnotationTemplate', $va_detail_config['options'], '^ca_representation_annotations.preferred_labels.name'));
		$ps_display_type		 			= caGetOption('display', $pa_options, false);
	 ...
	 */
	function manaGetMediaViewerHTML($po_request, $ps_identifier, $pt_subject, $pa_options=null) {
	 ...
	 * @throws ApplicationException
	 */
	function manaGetMediaViewerData($po_request, $ps_identifier, $pt_subject, $pa_options=null) {
		if (!($va_identifier = caParseMediaIdentifier($ps_identifier))) {
			throw new ApplicationException(_t('Invalid identifier %1', $ps_identifier));
		}
	 ...
	 * @throws ApplicationException
	 */
 	function manaRepresentationViewerHTMLBundles($po_request, $po_data, $pt_subject, $pa_options=null) {
	 ...
	 * @return string
	 */
	function manaGetTooltipJS($pa_tooltips, $ps_class = 'tooltipFormat') {
		$vs_buf = "<script type='text/javascript'>\njQuery(document).ready(function() {\n";
	 ...
	 * @return string
	 */
	function manaGetBundlePreviewForRelationshipBundle($pa_initial_values, $ps_delimiter='; ') {
	 ...
	 * @return string
	 */
	function manaDoTemplateTagSubstitution($po_view, $pm_subject, $ps_template_path, $pa_options=null) {
	 ...
	 * @return bool
	 */
	function manaDragAndDropSortingForHierarchyEnabled($pt_request, $ps_table, $pn_id=null) {
		$o_dm = Datamodel::load();
		$o_config = Configuration::load();
	 ...
	 */
	function manaGetDragAndDropSortingAvailabilityMap($pt_request, $ps_table, $pn_id) {
		$o_dm = Datamodel::load();
		$o_config = Configuration::load();
	 ...
	 */
	function manaExtractSettingValueByLocale($pa_settings, $ps_key, $ps_locale) {
