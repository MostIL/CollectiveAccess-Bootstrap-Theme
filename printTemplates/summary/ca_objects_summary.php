...
<?php
	foreach($va_placements as $vn_placement_id => $va_bundle_info){
		...
		preg_match_all('/./us', $va_bundle_info['display'], $va_b_i_d);
		$l = join('',array_reverse( $va_b_i_d[0]));
		preg_match_all('/./us', $vs_display_value, $vs_d_v);
		$v = join('',array_reverse( $vs_d_v[0]));
		print '<div class="data"><span class="label">'."{$l} </span><span class='meta'> {$v}</span></div>\n";
	}
