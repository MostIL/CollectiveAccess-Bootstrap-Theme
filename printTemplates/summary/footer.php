 ...
 */
	...
	if($this->request->config->get('summary_header_enabled')) {
		...
		if($this->request->config->get('summary_show_identifier')) {
			$vs_footer .= "<td class='footerText'  style='font-family: \"arial\"; font-size: 12px; text-align: center;'>".$t_item->getLabelForDisplay()." (".$t_item->get($t_item->getProperty('ID_NUMBERING_ID_FIELD')).")</td>";
		}
	
		if($this->request->config->get('summary_show_timestamp')) {
			$vs_footer .= "<td class='footerText'  style='font-family: \"arial\"; font-size: 12px; text-align: center;'>".caGetLocalizedDate(null, array('dateFormat' => 'delimited'))."</td>";
		}
		$vs_footer .= "</tr></table>";
